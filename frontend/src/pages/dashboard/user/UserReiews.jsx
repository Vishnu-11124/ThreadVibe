...
const UserReiews = () => {
    ...
    if(error) return <p className="text-center text-red-400 text-lg mt-20">No reviews found</p>
    
    const reviewData = reviews?.data
  return (
    <div className="p-4 sm:p-8 min-h-screen bg-gray-50">
      <div className="mb-8">
        <h2 className="text-3xl font-extrabold text-gray-900 tracking-tight">Your Reviews</h2>
      </div>
            ...
            reviewData.map((review, index) => (
                <div key={index} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden flex flex-col">
                    ...
                    <div className="p-4 flex flex-col gap-2 flex-1">
                        <p className="text-sm font-bold text-gray-900 truncate">{review.productId?.name}</p>
                        <p className="text-xs text-gray-300 font-mono truncate">{review.productId?._id}</p>
                        ...
                        <p className="text-xs text-gray-400 mt-auto pt-2 border-t border-gray-50">{new Date(review?.createdAt).toLocaleDateString()}</p>
                    </div>
                </div>
            ))
        }
